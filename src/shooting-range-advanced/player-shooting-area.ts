...
import { AudioSource, AvatarAnchorPointType, AvatarAttach, Entity, InputAction, Material, MeshCollider, MeshRenderer, PointerEventType, RaycastQueryType, Transform, engine, inputSystem, raycastSystem } from "@dcl/sdk/ecs";
...
export module PlayerShootingArea {
            ...
            function (raycastResult) {
                ...
                if (raycastResult.hits.length > 0) {
                    if (raycastResult.hits[0].entityId) {
                        ...
                        const hitPos = raycastResult.hits[0].position; if(!hitPos) return;
                        const hitID = raycastResult.hits[0].entityId; if(!hitID) return;
                        ...
                        const hitNormal = {x:raycastResult.hits[0].normalHit.x,y:raycastResult.hits[0].normalHit.y,z:raycastResult.hits[0].normalHit.z,w:0};
                        //calculate modified transform
                        const modTransform = CalculatePlacementDifference(tarEntity, {pos: hitPosition, rot: {x:0, y:0, z:0}});
                        const modQuart = Quaternion.fromEulerDegrees(modTransform.rot.x, modTransform.rot.y, modTransform.rot.z)
                        if(isDebugging) console.log("Shooting Area: hit at "
                            + "\n\tposition {x="+modTransform.pos.x+", y="+modTransform.pos.y+", z="+modTransform.pos.z+"}"
                            + "\n\trotation {x="+modTransform.rot.x+", y="+modTransform.rot.y+", z="+modTransform.rot.z+"}"
                        );
    ...
    function CalculatePlacementDifference(entity:Entity, transMod:ShootingAreaTransformData):ShootingAreaTransformData {
        ...
        if(transform.parent)
            CalculatePlacementDifference(transform.parent, transMod);
    ...
    export function GetSurfaceObject():Entity {
        ...
        if(shootingAreaEntity == undefined) {
            ...
            //  trigger area
            utils.triggers.addTrigger(shootingAreaEntity, utils.NO_LAYERS, utils.LAYER_1, 
