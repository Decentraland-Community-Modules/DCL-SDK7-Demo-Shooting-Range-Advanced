...
import { Schemas, engine, Entity, Transform, GltfContainer, ColliderLayer, DeepReadonlyObject, MeshRenderer, MeshCollider } from "@dcl/sdk/ecs";
...
export module TargetObject {
	...
	export function Create(data:TargetDataObject):TargetObjectPiece {
		if(isDebugging) console.log("Target Object: attempting to create object of type="+data.type+" at pos(x="+data.pos.x+", y="+data.pos.y+", z="+data.pos.z+")...");
		...
		if(def.rotatingPieces) {
			...
			for (let i = 0; i < rotatingPieceCount; i++) {
				...
				//	update component
				utils.perpetualMotions.startRotation(entityRotating, Quaternion.fromEulerDegrees(
		...
		if(isDebugging) console.log("Target Object: object id="+entry.EntityParent.toString()+" placed, total objects="+pooledObjects.length);
		return entry;
  	}
