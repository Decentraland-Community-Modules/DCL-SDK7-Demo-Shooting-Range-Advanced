...
export module ShotDecalObject {
	...
	export function Create(data:ShotDecalDataObject):Entity {
		if(isDebugging) console.log("Shot Decal Object: attempting to create object at pos(x="+data.pos.x+", y="+data.pos.y+", z="+data.pos.z+")...");
        ...
        utils.timers.setTimeout( function() { engine.removeEntity(anchor) }, DECAL_LIFESPAN );

		//add entity to pooling
		pooledObjects.push(entity);
		if(isDebugging) console.log("Shot Decal Object: created new object at"
        + "\n\tpar rotation {x="+data.pivot.x+", y="+data.pivot.y+", z="+data.pivot.z+"}"
        + "\n\ttar position {x="+data.pos.x+", y="+data.pos.y+", z="+data.pos.z+"}"
        + "\n\ttar rotation {x="+data.rot.x+", y="+data.rot.y+", z="+data.rot.z+"}"
    );
